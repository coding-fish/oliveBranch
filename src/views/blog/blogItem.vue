        ...
        <el-col :span="5">
          <el-card shadow="never">
            <div class="flex6" style="margin-left:16px">
              ...
              <el-link class="blog-title" :underline="false" :href="'/userinfo/'+userid" style="margin-left:10px">
                {{ username }}
              </el-link>
            </div>
          ...
          <el-card shadow="never" style="margin-top:30px">
            ...
            <ul v-for="(item,index) in userhotlist" :key="index">
              <li style="margin-top:5px;margin-left:10px;margin-bottom:10px" class="flex6">
                <el-link class="blog-title" :underline="false" :href="'/blogItem/'+userid+'/'+item.blogid"
                         style="font-size:12px;font-weight:500;color:gray;margin-right:5px">{{ item.blogname }}
                ...
                <li style="margin-top:5px;margin-left:10px;margin-bottom:0px">
                  <div style="margin-bottom:40px">
                    <div style="margin-bottom:0px">
                      <div>
                        <div class="flex7">
                          <el-image style="width:40px;height:40px;border-radius:40px" :src="item.img"
                                    fit="cover"></el-image>
                          <el-link class="blog-title" :underline="false" :href="'/userinfo/'+item.user_id"
                                   style="margin-left:10px;align-items:top !important">{{ item.name }} :
                          </el-link>

                        </div>
                        <el-button type="text" style="float:right;margin-top:-40px"
                                   @click="tipid=item.comment_id;dialogVisible2 = true">
                          <svg class="icon color_deep iconmargin" aria-hidden="true" style="font-size:20px">
                            <use xlink:href="#icon-report"></use>
                          </svg>
                        </el-button>
              ...
              <ul v-for="(item,index) in hotbloglist" :key="index">
                <li style="margin-top:5px;margin-left:10px">
                  ...
                  <div>
                    ...
                    <div class="flex6">
                      <p style="width:50%;padding-left:15px;white-space:nowrap;font-size:14px;color:gray;overflow: hidden; text-overflow: ellipsis;">
                        {{ item.textcontent }}</p>
  ...
  data() {
    ...
    this.$axios.post('/apis/blog/getuserbloginfo',
        ...
        .then(res => {
          ...
          this.userlist = [{num: res.data.blogNum, tip: "贴子"}, {
            num: res.data.likeNum,
            tip: "获赞"
          }, {num: res.data.tipNum, tip: "评论"},]
        })
    //获取关注状态
